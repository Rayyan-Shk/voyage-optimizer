...
from src.core.models import MaintenanceForecasting, MaintenanceRecommendation, APIResponse
...
async def get_maintenance_alerts(
    ...
    try:
        ...
        historical_maintenance = await maintenance_data_service.get_historical_maintenance(ship_id, db)
        ...
        logger.info(
            ...
            next_critical_date=forecast.next_critical_date.isoformat() if forecast.next_critical_date else None
        )
        ...
        raise HTTPException(status_code=400, detail=f"Maintenance forecasting failed: {str(e)}")
    
    except Exception as e:
        logger.error("Failed to generate maintenance alerts", error=str(e), exc_info=True)
        raise HTTPException(status_code=500, detail="Failed to generate maintenance alerts")


@router.get("/maintenance-alerts/{ship_id}/component/{component}", response_model=List[MaintenanceRecommendation])
async def get_component_maintenance_alerts(
    ...
    try:
        ...
        component_recommendations = [
            rec for rec in forecast.recommendations
        ...
        raise HTTPException(status_code=500, detail="Failed to get component maintenance alerts")
    ...
    try:
        ...
        for i in range(min(limit, 10)):
            event = {
                "id": f"maint_{i}",
                "ship_id": str(ship_id),
                "component": ["Engine", "Propulsion", "Navigation", "Electrical"][i % 4],
                "maintenance_type": ["routine", "preventive", "emergency"][i % 3],
                "scheduled_date": (datetime.now() - timedelta(days=i*30)).isoformat(),
                "completed_date": (datetime.now() - timedelta(days=i*30-2)).isoformat(),
                "cost": 5000 + (i * 1000),
        ...
        raise HTTPException(status_code=500, detail="Failed to Fetch the Maintanence History")
...
async def get_maintenance_analytics(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Failed to generate maintenance analytics")
