...
from src.core.models import (
    VoyagePlanRequest, VoyagePlanResponse, VoyageHistory, 
    AlternativePlan, APIResponse, Route, Coordinates, VoyageStatus
)
from src.core.exceptions import RouteOptimizationError, FuelPredictionError
...
async def plan_voyage(
    ...
    try:
        ...
        ship_specs = await maintenance_data_service.get_ship_specifications(request.ship_id, db)
        ...
        route_data = {
            ...
            "average_speed": sum(wp.speed for wp in route_optimization.waypoints) / len(route_optimization.waypoints),
        ...
        from src.services.maintenance_data_service import maintenance_data_service
        usage_data = await maintenance_data_service.get_ship_usage_data(request.ship_id, db)
        historical_maintenance = await maintenance_data_service.get_historical_maintenance(request.ship_id, db)
        ...
        usage_data.update({
            ...
            "weather_severity_avg": weather_data.get("weather_severity", 0.5) if weather_data else 0.5
        })
        ...
        total_estimated_cost = fuel_prediction.estimated_consumption * 0.5  # Assume $0.5 per liter
        if maintenance_forecast.recommendations:
            total_estimated_cost += sum(rec.estimated_cost for rec in maintenance_forecast.recommendations)
        
        # Calculate overall confidence score
        # Extract confidence from fuel prediction (use inverse of relative error as confidence)
        fuel_confidence = 1.0 - (fuel_prediction.confidence_interval['std_error'] / fuel_prediction.estimated_consumption)
        fuel_confidence = max(0.0, min(1.0, fuel_confidence))  # Clamp between 0 and 1
        
        confidence_score = (
            route_optimization.confidence_score * 0.4 +
            fuel_confidence * 0.4 +
            (sum(rec.urgency_score for rec in maintenance_forecast.recommendations) / len(maintenance_forecast.recommendations) if maintenance_forecast.recommendations else 0.8) * 0.2
        )
        ...
        raise HTTPException(status_code=400, detail=f"Route optimization failed: {str(e)}")
...
async def generate_alternative_plans(
    ...
    speed_fuel = fuel_predictor.predict(
        ...
        route_data={
            ...
            "average_speed": sum(wp.speed for wp in speed_route.waypoints) / len(speed_route.waypoints),
    ...
    fuel_fuel = fuel_predictor.predict(
        ...
        route_data={
            ...
            "average_speed": sum(wp.speed for wp in fuel_route.waypoints) / len(fuel_route.waypoints),
