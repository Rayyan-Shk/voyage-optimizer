...
class WeatherClient:
    ...
    async def get_forecast(self, coordinates: Coordinates, days: int = 5) -> Optional[Dict]:
        """Get weather forecast for specific coordinates."""
        try:
            cache_key = f"forecast_{coordinates.latitude}_{coordinates.longitude}_{days}"
    ...
    async def get_route_weather(self, origin: Coordinates, destination: Coordinates) -> Optional[Dict]:
        ...
        try:
            ...
            logger.info("Route weather fetched successfully", waypoints_count=len(waypoints))
        ...
        try:
            ...
            return {
                "location": {
                    ...
                    "longitude": raw_data.get("city", {}).get("coord", {}).get("lon", 0)
                },
    ...
    def _generate_route_waypoints(self, origin: Coordinates, destination: Coordinates, num_points: int = 5) -> List[Coordinates]:
    ...
    def _aggregate_route_weather(self, weather_data: List[Dict]) -> Dict:
        ...
        avg_wind_speed = sum(w.get("wind_speed", 10) for w in weather_data) / len(weather_data)
        avg_wave_height = sum(w.get("wave_height", 2) for w in weather_data) / len(weather_data)
        avg_visibility = sum(w.get("visibility", 10) for w in weather_data) / len(weather_data)
        ...
        return {
            ...
            "wind_direction": weather_data[0].get("wind_direction", 180),  # Use first waypoint
    ...
    def _calculate_weather_severity(self, weather_data: Dict) -> float:
        ...
        try:
            ...
            # Combined severity
            severity = (wind_factor * 0.4 + condition_factor * 0.4 + visibility_factor * 0.2)
    ...
    def _get_fallback_weather(self, coordinates: Coordinates) -> Dict:
        ...
        logger.warning("Using fallback weather data", lat=coordinates.latitude, lon=coordinates.longitude)
        return self._get_default_weather()
    ...
    def _get_fallback_route_weather(self, origin: Coordinates, destination: Coordinates) -> Dict:
