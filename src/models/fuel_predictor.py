...
from src.core.models import FuelPrediction, Coordinates, WeatherCondition
...
class FuelPredictor:
    ...
    def __init__(self):
        ...
        self.poly_features = PolynomialFeatures(degree=poly_config['polynomial_degree'], 
    ...
    def _generate_synthetic_features(self, n_samples: int) -> pd.DataFrame:
        ...
        data['cargo_weight'] = np.clip(data['cargo_weight'], 0, data['displacement'] * 0.8)
    ...
    def _generate_synthetic_fuel_consumption(self, features: pd.DataFrame) -> np.ndarray:
        """Generate realistic synthetic fuel consumption data."""
        # Base consumption factors
        base_consumption = (
            features['distance'] * 0.1 +  # Distance factor
            features['displacement'] * 0.0001 +  # Ship size factor
            features['engine_power'] * 0.0002 +  # Engine power factor
            features['average_speed'] ** 2 * 0.5  # Speed factor (quadratic)
        )
        ...
        # Cargo impact
        cargo_impact = (
            features['cargo_weight'] * 0.0003 +
            features['cargo_density'] * 10
        )
    ...
    def extract_features(self, ship_specs: Dict, route_data: Dict, 
        ...
        features = feature_engineer.extract_fuel_features(ship_specs, route_data, weather_conditions)
        
        # Add fuel-specific features
        features['fuel_efficiency_rating'] = ship_specs.get('fuel_efficiency_rating', 0.8)
    ...
    def _calculate_weather_severity_REMOVED(self, weather_conditions: Dict) -> float:
        ...
        # Weighted average
        severity = (wind_factor * 0.3 + wave_factor * 0.4 + 
                   precip_factor * 0.2 + visibility_factor * 0.1)
    ...
    def predict(self, ship_specs: Dict, route_data: Dict, 
                weather_conditions: Optional[Dict] = None) -> FuelPrediction:
        ...
        try:
            ...
            feature_df = data_validator.validate_model_input(features, self.feature_names)
    ...
    def _calculate_confidence_interval(self, features_poly: np.ndarray) -> Dict[str, float]:
    ...
    def update_model(self, new_features: pd.DataFrame, new_fuel_consumption: np.ndarray):
        ...
        try:
            ...
            alpha = 0.1
            self.model_metrics['mae'] = (1 - alpha) * self.model_metrics['mae'] + alpha * new_mae
