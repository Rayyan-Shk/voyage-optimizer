...
class FeatureEngineeringService:
    ...
    def extract_maintenance_features(self, ship_id: str, current_date: datetime,
                                   usage_data: Dict[str, Any], 
                                   historical_maintenance: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            validated_maintenance = data_validator.validate_maintenance_history(historical_maintenance)
            ...
            features.update(self._extract_maintenance_history_features(validated_maintenance, current_date))
            
            # Component-specific features
            features.update(self._extract_component_features(validated_maintenance, current_date))
            ...
            raise FeatureEngineeringError(f"Failed to extract maintenance features: {str(e)}")
    
    def extract_fuel_features(self, ship_specs: Dict[str, Any], route_data: Dict[str, Any],
                            weather_conditions: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            # Weather features
            features.update(self._extract_weather_condition_features(weather_conditions))
            
            self.logger.info(
                "Fuel features extracted",
                feature_count=len(features)
            )
    ...
    def _extract_maintenance_history_features(self, maintenance_history: List[Dict[str, Any]], 
        ...
        return {
            ...
            'maintenance_frequency': self._calculate_maintenance_frequency(maintenance_history),
            'average_maintenance_cost': self._calculate_average_maintenance_cost(maintenance_history),
            'emergency_maintenance_ratio': self._calculate_emergency_ratio(maintenance_history),
        }
    
    def _extract_component_features(self, maintenance_history: List[Dict[str, Any]], 
                                  current_date: datetime) -> Dict[str, Any]:
        """Extract component-specific features."""
        features = {}
        
        for component_category in self.component_categories:
            features[f'{component_category}_last_maintenance'] = self._get_component_last_maintenance(
                maintenance_history, component_category, current_date
            )
            features[f'{component_category}_failure_history'] = self._get_component_failure_history(
    ...
    def _extract_cargo_features(self, ship_specs: Dict[str, Any], route_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _extract_weather_condition_features(self, weather_conditions: Optional[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if weather_conditions:
            return {
                ...
                'weather_severity': self._calculate_weather_severity(weather_conditions),
                'headwind_percentage': weather_conditions.get('headwind_percentage', 0.5),
            }
        else:
    ...
    def _calculate_days_since_last_maintenance(self, maintenance_history: List[Dict[str, Any]],
                                             current_date: datetime) -> int:
        """Calculate days since last maintenance."""
        if not maintenance_history:
            return 365  # Default if no history
        
        last_maintenance = max(maintenance_history, key=lambda x: x.get('date', datetime.min))
        last_date = last_maintenance.get('date', current_date - timedelta(days=365))
        
        return (current_date - last_date).days
    
    def _calculate_maintenance_frequency(self, maintenance_history: List[Dict[str, Any]]) -> float:
        """Calculate maintenance frequency (events per year)."""
        if len(maintenance_history) < 2:
            return 4.0  # Default frequency
        
        # Sort by date
        sorted_history = sorted(maintenance_history, key=lambda x: x.get('date', datetime.min))
    ...
    def _calculate_average_maintenance_cost(self, maintenance_history: List[Dict[str, Any]]) -> float:
    ...
    def _calculate_emergency_ratio(self, maintenance_history: List[Dict[str, Any]]) -> float:
        """Calculate ratio of emergency maintenance events."""
        if not maintenance_history:
            return 0.1  # Default ratio
        
        emergency_count = sum(1 for event in maintenance_history 
                            if event.get('event_type') == 'emergency')
        
        return emergency_count / len(maintenance_history)
    
    def _get_component_last_maintenance(self, maintenance_history: List[Dict[str, Any]], 
                                      component_category: str, current_date: datetime) -> int:
        """Get days since last maintenance for specific component category."""
        component_events = [
            event for event in maintenance_history
    ...
    def _get_component_failure_history(self, maintenance_history: List[Dict[str, Any]], 
                                     component_category: str) -> float:
        ...
        emergency_count = sum(1 for event in component_events 
                            if event.get('event_type') == 'emergency')
