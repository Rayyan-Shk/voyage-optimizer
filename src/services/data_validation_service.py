...
class DataValidationService:
    ...
    def validate_ship_usage_data(self, usage_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if validated_data.recent_operating_hours > validated_data.total_operating_hours:
                self.logger.warning(
            ...
            if validated_data.fuel_efficiency < 0.3:
                self.logger.warning("Very low fuel efficiency detected", efficiency=validated_data.fuel_efficiency)
            
            if validated_data.condition_score < 0.5:
                self.logger.warning("Poor ship condition detected", condition=validated_data.condition_score)
    ...
    def validate_maintenance_history(self, maintenance_history: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            for event in validated_history:
                ...
                if event_key not in seen_events:
                    ...
                    self.logger.warning("Duplicate maintenance event removed", event=event_key)
    ...
    def validate_feature_data(self, features: Dict[str, Any], required_features: List[str]) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for feature_name, value in features.items():
                if value is None:
                    self.logger.warning(f"Null value for feature {feature_name}, setting to 0")
                    validated_features[feature_name] = 0.0
                elif isinstance(value, (int, float)):
                    if np.isnan(value) or np.isinf(value):
                        self.logger.warning(f"Invalid numeric value for feature {feature_name}, setting to 0")
                        validated_features[feature_name] = 0.0
                    else:
                    ...
                    try:
                        ...
                        self.logger.warning(f"Cannot convert feature {feature_name} to float, setting to 0")
                        validated_features[feature_name] = 0.0
    ...
    def validate_model_input(self, data: Union[Dict, pd.DataFrame], 
        ...
        try:
            ...
            for col in expected_columns:
                if col not in df.columns:
                    self.logger.warning(f"Missing column {col}, adding with default value 0")
                    df[col] = 0.0
            ...
            for col in df.columns:
                if not pd.api.types.is_numeric_dtype(df[col]):
                    try:
                        ...
                        self.logger.warning(f"Cannot convert column {col} to numeric, filling with 0")
                        df[col] = 0.0
