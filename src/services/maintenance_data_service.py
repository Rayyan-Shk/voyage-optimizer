...
class MaintenanceDataService:
    ...
    async def get_ship_usage_data(self, ship_id: UUID, db: Session) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.error(
                "Failed to get ship usage data", ship_id=str(ship_id), error=str(e)
            )
            raise DatabaseError(f"Failed to get ship usage data: {str(e)}")

    async def get_historical_maintenance(
        self, ship_id: UUID, db: Session, limit: int = 100
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            maintenance_events = (
                ...
                .filter(MaintenanceEvent.ship_id == ship_id)
                .filter(MaintenanceEvent.actual_date.isnot(None))
                .order_by(desc(MaintenanceEvent.actual_date))
                .limit(limit)
                .all()
            )
            ...
            # Validate the data
            validated_history = data_validator.validate_maintenance_history(
                maintenance_history
            )
            ...
            self.logger.error(
                "Failed to get historical maintenance",
                ship_id=str(ship_id),
                error=str(e),
            )
            raise DatabaseError(f"Failed to get historical maintenance: {str(e)}")

    async def get_ship_specifications(
        self, ship_id: UUID, db: Session
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            self.logger.error(
                "Failed to get ship specifications", ship_id=str(ship_id), error=str(e)
            )
            raise DatabaseError(f"Failed to get ship specifications: {str(e)}")

    async def store_maintenance_prediction(
        self, ship_id: UUID, prediction_data: Dict[str, Any], db: Session
    ) -> None:
        """
        ...
        try:
            ...
            db.rollback()
            self.logger.error(
                "Failed to store maintenance prediction",
                ship_id=str(ship_id),
                error=str(e),
            )
    ...
    async def get_maintenance_statistics(
        ...
        try:
            ...
            total_cost = sum(
                float(event.cost) if event.cost else 0.0 for event in events
            )
            average_cost = total_cost / total_events if total_events > 0 else 0.0

            emergency_events = sum(
                1 for event in events if event.maintenance_type == "emergency"
            )
            emergency_ratio = (
                emergency_events / total_events if total_events > 0 else 0.0
            )
            ...
            self.logger.info(
                "Maintenance statistics calculated", ship_id=str(ship_id), **statistics
            )
