...
class WeatherCondition(BaseModel):
    ...
    weather_type: str = Field(..., description="Weather type (clear, cloudy, stormy, etc.)")
...
class Route(BaseModel):
    ...
    total_distance: float = Field(..., ge=0, description="Total distance in nautical miles")
    ...
    confidence_score: float = Field(..., ge=0, le=1, description="Route confidence score")


class RouteOptimization(BaseModel):
    """Route optimization result with detailed waypoints."""
    waypoints: List[Waypoint]
    total_distance: float = Field(..., gt=0, description="Total distance in nautical miles")
...
class FuelPrediction(BaseModel):
    ...
    estimated_consumption: float = Field(..., ge=0, description="Estimated fuel consumption in tons")
    confidence_interval: Dict[str, float] = Field(
        default_factory=lambda: {"lower": 0, "upper": 0}
    )
    factors: Dict[str, float] = Field(default_factory=dict)
    efficiency_score: float = Field(..., ge=0, le=1, description="Fuel efficiency score")
...
class MaintenanceForecasting(BaseModel):
    ...
    confidence_score: float = Field(..., ge=0, le=1, description="Forecasting confidence score")
    next_critical_date: Optional[datetime] = None
...
class VoyageFeedback(BaseModel):
    ...
    voyage_id: UUID
    ship_id: UUID
    actual_fuel_consumption: float = Field(..., ge=0, description="Actual fuel consumption in tons")
    ...
    weather_accuracy: float = Field(0.85, ge=0, le=1, description="Weather forecast accuracy")
    maintenance_events: List[MaintenanceRecommendation] = Field(default_factory=list)
    overall_satisfaction: float = Field(..., ge=1, le=5, description="Overall satisfaction (1-5)")
