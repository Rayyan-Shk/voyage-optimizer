...
class Ship(Base):
    ...
    voyages = relationship(
        "Voyage", back_populates="ship", cascade="all, delete-orphan"
    )
    fuel_logs = relationship(
        "FuelLog", back_populates="ship", cascade="all, delete-orphan"
    )
    maintenance_events = relationship(
        "MaintenanceEvent", back_populates="ship", cascade="all, delete-orphan"
    )
...
class Voyage(Base):
    ...
    fuel_logs = relationship(
        "FuelLog", back_populates="voyage", cascade="all, delete-orphan"
    )
...
class FuelLog(Base):
    ...
    __table_args__ = (
        ...
        Index(
            "idx_fuel_logs_weather_gin", "weather_conditions", postgresql_using="gin"
        ),
...
class MaintenanceEvent(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "maintenance_type IN ('routine', 'preventive', 'emergency', 'overhaul')"
        ),
...
class ModelPerformance(Base):
    ...
    __table_args__ = (
        ...
        Index(
            "idx_model_performance_metrics_gin",
            "accuracy_metrics",
